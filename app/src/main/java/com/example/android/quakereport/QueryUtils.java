...
/**
 * Created by soumyajit on 14/8/17.
 */
...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
                JSONObject rootEarthQuake = new JSONObject(SAMPLE_JSON_RESPONSE);
                JSONArray  featuresArray = rootEarthQuake.getJSONArray("features");
                for(int i =0 ; i < featuresArray.length() ; i++) {
                    // we are creating a json onject which create a object for every objects in the features array

                    JSONObject objIndexFeatures = featuresArray.getJSONObject(i);
